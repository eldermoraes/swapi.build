...
export function renderWebMcp(container: HTMLElement, status: RegistrationStatus): void {
  container.innerHTML = `
    <section class="docs sw-inner-prose webmcp-guide">
      ...
      <p>WebMCP is an evolving browser proposal. Use a compatible browser with WebMCP enabled; it is not available in every browser or assistant. See the <a href="https://developer.chrome.com/docs/ai/webmcp" target="_blank" rel="noopener noreferrer">Chrome setup and inspector guide</a>. For local testing, enable <code>chrome://flags/#enable-webmcp-testing</code> and relaunch the browser.</p>
      ...
      <p>Validated with Chrome 152 and the WebMCP testing flag. In this version the browser may omit execution cancellation options; user navigation and typing still supersede pending page actions.</p>
      <h2>Tools</h2>
      <div class="sw-table-wrap"><table>
        <thead><tr><th>Name</th><th>Arguments</th><th>Visible result</th></tr></thead>
        <tbody>
          <tr><td><code>sw_list</code></td><td>resource</td><td>Show all records and clear the previous search.</td></tr>
          <tr><td><code>sw_get</code></td><td>resource, id</td><td>Open a record's details.</td></tr>
          <tr><td><code>sw_search</code></td><td>resource, query</td><td>Fill the search and show matching records.</td></tr>
          <tr><td><code>sw_random</code></td><td>resource</td><td>Open the selected random record.</td></tr>
        </tbody>
      </table></div>
      <p>Use <code>PEOPLE</code>, <code>FILMS</code>, <code>PLANETS</code>, <code>SPECIES</code>, <code>STARSHIPS</code> or <code>VEHICLES</code>. IDs come from record URLs: <code>FILMS</code> ID <code>1</code> is A New Hope, not episode I. Search matches name fragments (titles for films), ignores case and accepts an empty string to match all records.</p>
      <p>Names and arguments match the remote MCP tools. WebMCP additionally changes this page and returns a JSON string with <code>ok</code>, complete records in <code>data</code>, <code>resource</code>, <code>path</code> and either <code>count</code> or <code>id</code>. Its response envelope is specific to this browser integration.</p>
      ...
      <p>With a compatible agent connected, try “List the characters”, “Find Luke Skywalker and open his details”, or “Show me a random starship”. Agent availability and any model configuration belong to your browser or inspector.</p>
      <h2>You control the page</h2>
      <p>All tools only read server data, but they update the visible page. Only one agent action runs at a time; another receives <code>BUSY</code>. Typing or navigating takes priority over pending work. A cancelled browser call may end without a JSON response. Other errors include <code>INVALID_ARGUMENT</code>, <code>NOT_FOUND</code>, <code>NETWORK_ERROR</code>, <code>HTTP_ERROR</code> and <code>INVALID_RESPONSE</code>.</p>
      ...
      <a class="sw-pill sw-pill--solid" href="/resource/people">Open the explorer</a>
    </section>`;
